...
class GeneralSetup extends DSPModule {
  ...
  val prevRadStageSum = Vec(List(DSPUInt(0)) ++ io.radStageSum.init)
    ...
    if (possibleRad.contains(2) && possibleRad.contains(4)) {
      Vec(io.radStageSum.zipWithIndex.map { case (e, i) => {
        ...
        if (e == io.radStageSum.last) e
          ...
          val is4LocUse2 = is4Loc & io.use2
          Mux(is4LocUse2, io.radStageSum(i + 1), e).pipe(1)
        }
      }})
    }
    else io.radStageSum
    ...
    if (possibleRad.contains(2) && possibleRad.contains(4)) {
      Vec(prevRadStageSum.zipWithIndex.map { case (e, i) => {
        ...
        if (e == prevRadStageSum.head) e
          ...
          Mux(is2LocUse4, prevRadStageSum(i - 1), e).pipe(1)
        }
      }})
    }
  ...
  val (primeStageSumX,prevPrimeStageSumX) = (0 until Params.getCalc.maxStages).map(i => {
    val cond = io.radStageSum.init.map(x => DSPUInt(i) < x)
    val o1 = {
      ...
      if(i == Params.getCalc.maxStages-1) primeStageSumShort.last
        ...
        (0 until io.radStageSum.length-1).foldRight(primeStageSumShort.last)(
          (e,accum) => Mux(cond(e),primeStageSumShort(e),accum).pipe(1)
        )
      }
    }
    val o2 = (0 until io.radStageSum.length-1).foldRight(prevPrimeStageSumShort.last)(
      (e,accum) => Mux(cond(e),prevPrimeStageSumShort(e),accum).pipe(1)
    )
    (o1,o2)
  }).unzip
  io.primeStageSum := Vec(primeStageSumX)
  io.prevPrimeStageSum := Vec(prevPrimeStageSumX)
