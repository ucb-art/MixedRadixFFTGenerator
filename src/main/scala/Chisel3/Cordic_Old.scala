...
// x + i * y
case class CordicParamsO[T <: Data:RealBits](xyType: T, unrollingFactor: Double) {
...
class CordicIOCoreO[T <: Data:RealBits](cordicParams: CordicParamsO[T]) extends Bundle {
  ...
  override def cloneType = (new CordicIOCoreO(cordicParams)).asInstanceOf[this.type]
}

class CordicIOO[T <: Data:RealBits](cordicParams: CordicParamsO[T]) extends Bundle {
  val in = new CordicIOCoreO(cordicParams)
  val out = Flipped(new CordicIOCoreO(cordicParams))
  val clk = Input(Clock())
  val reset = Input(Bool())
  override def cloneType = (new CordicIOO(cordicParams)).asInstanceOf[this.type]
}
...
// z' = z - Pi
@chiselName
class CordicO[T <: Data:RealBits](cordicParams: CordicParamsO[T]) extends Module {
  ...
  val cordicStages = stageIterations.zip(stageOffset).map { case (iterations, offset) => 
    Module(new CordicStageO(sintCordicParams, offset = offset, iterations = iterations))
  }
...
@chiselName
class CordicStageO[T <: Data:RealBits](cordicParams: CordicParamsO[T], offset: Int, iterations: Int) extends Module {
  require(offset >= 0)
...
class CordicSpecO extends FlatSpec with Matchers {
  behavior of "Cordic"
  it should "pass tests" in {
    ...
    val opt = TestParams.optionsBTolWaveformTB(
      lsbs = 8, 
      outDir = "test_run_dir/CordicTBO", 
      genVerilogTB = false
    )
    val params = CordicParamsO(
      xyType = FixedPoint(23.W, 21.BP),
      unrollingFactor = 0
    )
    
    dsptools.Driver.execute(() => 
      new CordicWrapperO(params), opt
    ) { c =>
      new CordicTesterO(c)
    } should be (true)
