  ...
  def apply(FFTN : Int, frames: Int) : Tuple2[List[ScalaComplex],List[ScalaComplex]] = {
    ...
    // Larger FFT sizes
      ...
      for (i <- 0 until FFTN){
        ...
        val r2 = r1.foldLeft(i.toDouble/FFTN/100)(_ + _)
        inProto = inProto :+ Complex(r2 + 0.001*Random.nextGaussian,0.004*Random.nextGaussian)
      }
    }
    // Direct (inefficient) FFT calculation
    // exp(ix) = cos(x) + i*sin(x)
    // exp(-j*2*pi*n*k/FFTN) = cos(-2*pi*n*k/FFTN) + i*sin(-2*pi*n*k/FFTN)
    for (k <- 0 until inProto.length;
         n <- 0 until inProto.length){
    ...
    val inN = List.fill(frames)(inProto.toList).flatten
    val outN = List.fill(frames)(outProto.toList).flatten
    (inN,outN)
  }
